...
lifeWin::lifeWin()
	: BDirectWindow(BRect(100,100,510,510), "3D Life", B_TITLED_WINDOW, 0)
//for voodoo change to a BDirectWindow
{

	mv = new lifeView(BRect(0,20,410,410));
	AddChild(mv);

	mv->EnableDirectMode(true); //for voodoo
	...
	menu->AddItem(new BMenuItem("Full Screen", new BMessage(FULL_SCREEN), 'F')); //voodoo
...
//for voodoo we need to catch DirectConnected and pass along
void lifeWin::DirectConnected(direct_buffer_info *info){

	mv->DirectConnected(info);
