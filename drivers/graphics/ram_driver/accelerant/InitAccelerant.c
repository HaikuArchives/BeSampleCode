...
status_t INIT_ACCELERANT(int the_fd) {
	...
	*/
	...
	si->mem_size = 1024 * 1024;	/* we only support 1MB right now */
	si->pix_clk_max8 = si->pix_clk_max16 = si->pix_clk_max32 = 360000; /* a very fast DAC */
	...
	*/
	/* we're going to lie and say we have hardware cursor support, but only
	implement stubs */
	si->cursor.data = 0;
